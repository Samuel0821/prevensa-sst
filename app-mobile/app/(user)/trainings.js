...
// Funcionalidad ajustada: El usuario puede crear, editar y cambiar el estado, pero no eliminar.
...
// --- PANTALLA DE CAPACITACIONES (VERSIÓN USUARIO CON EDICIÓN) ---
export default function TrainingsScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = { ...formState, description: formState.topic }; // Description se autogenera
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Gestión de Capacitaciones</Text>
      <FlatList
        data={trainings}
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.cardDescription}>{item.description || "Sin descripción detallada."}</Text>
            ...
            <View style={styles.cardActions}>
              ...
              {/* El botón de eliminar se ha omitido intencionalmente para el rol de usuario */}
            </View>
          </View>
        )}
        ListEmptyComponent={<Text style={styles.emptyText}>No hay capacitaciones programadas.</Text>}
      />
      {/* Botón flotante para crear una nueva capacitación */}
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: 'white', marginVertical: 8, marginHorizontal: 12, borderRadius: 10, elevation: 3, paddingVertical: 15 },
  ...
  cardActions: { flexDirection: 'row', justifyContent: 'flex-end', paddingHorizontal: 10, paddingTop: 10, alignItems: 'center' },
  actionButton: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10 },
  actionButtonText: { marginLeft: 8, color: '#007AFF', fontWeight: '600' },
