      ...
      <ScrollView contentContainerStyle={styles.modalContainer}>
        ...
        <TouchableOpacity onPress={() => setShowDatePicker(true)} style={styles.input}>
            <View style={styles.datePickerContainer}>
              <Text>{incident.date.toLocaleDateString('es-ES')}</Text>
...
// --- PANTALLA PRINCIPAL DE INCIDENTES (VERSIÓN USUARIO) ---
export default function IncidentsScreen() {
  ...
  const handleSave = async () => {
    ...
    if (formState.photo) {
      ...
      const fileName = `incident_${Date.now()}.${fileType}`;
  ...
  // Permite al usuario cambiar el estado de un incidente
  const toggleStatus = async (incident) => {
      ...
      try {
          ...
          Alert.alert("Estado Actualizado", `El incidente ahora está ${newStatus}.`);
      } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<View style={styles.centered}><Text style={styles.emptyText}>No hay incidentes registrados.</Text></View>}
      />
...
const styles = StyleSheet.create({
  ...
  header: { fontSize: 26, fontWeight: 'bold', padding: 20, backgroundColor: 'white', borderBottomWidth: 1, borderBottomColor: '#ddd' },
  card: { backgroundColor: 'white', marginVertical: 8, marginHorizontal:12, borderRadius: 10, elevation: 2, shadowColor:'#000', shadowOpacity:0.1, shadowRadius:4, shadowOffset:{width:0, height:2} },
  incidentImage: { width: '100%', height: 200, borderTopLeftRadius: 10, borderTopRightRadius: 10 },
  cardContent: { padding: 15 },
  cardTitle: { fontSize: 16, fontWeight: 'bold', marginBottom: 10 },
  cardInfo: { fontSize: 14, color: '#444', marginBottom: 5 },
  cardFooter: { flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', marginTop: 15 }, // Alineado a la izquierda
  ...
  modalContainer: { flexGrow: 1, justifyContent: 'center', padding: 25, backgroundColor: '#F9F9F9' },
